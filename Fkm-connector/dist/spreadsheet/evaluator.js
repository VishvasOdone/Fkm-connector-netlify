 ...
 */
import { cellKey, colLetter, rcKey } from './refs.js';
import { parse } from './parser.js';
import { E_CYC, E_DIV, E_EXPR, E_NA, E_NAME, E_NUM, E_REF, E_VAL, asNum, asText, compare, doRound, flatten, isErr, toNum, truthy, } from './values.js';
export function cellValue(book, sheet, rc) {
    const ck = cellKey(sheet, rc);
    if (book.cache.has(ck))
        return book.cache.get(ck);
    if (book.busy.has(ck))
        return E_CYC;
    ...
    if (cell.k === 'L') {
        ...
        const out = n !== null ? n : cell.v;
        book.cache.set(ck, out);
        return out;
    }
    book.busy.add(ck);
    const ast = parse(cell.toks);
    const res = ast === null ? E_EXPR : ev(book, ast, sheet);
    book.busy.delete(ck);
    ...
    const out = res === null ? 0 : res;
    book.cache.set(ck, out);
    return out;
}
export function ev(book, node, sheet) {
    switch (node.t) {
        case 'lit':
            return node.v;
        case 'blank':
...
function evBin(book, node, sheet) {
    const op = node.op;
...
function evShaped(book, node, sheet) {
    if (node.t === 'rng') {
        const sh = node.sheet !== null ? node.sheet : sheet;
        ...
        for (let r = r0; r <= r1; r++) {
            for (let c = c0; c <= c1; c++)
                vals.push(cellValue(book, sh, [r, c]));
        }
        return { rows: r1 - r0 + 1, cols: c1 - c0 + 1, vals };
    }
    const v = ev(book, node, sheet);
    if (Array.isArray(v))
        return { rows: 1, cols: v.length, vals: v };
...
function scalar(v) {
    ...
    return flat.length && !Array.isArray(flat[0]) ? flat[0] : null;
}
...
function evCall(book, node, sheet) {
    ...
    if (name === 'IFERROR') {
        ...
        if (isErr(v))
            return args.length > 1 ? ev(book, args[1], sheet) : '';
        return v;
    }
    ...
    if (name === 'ISNUMBER' || name === 'ISTEXT' || name === 'ISBLANK' || name === 'ISERROR' || name === 'ISNA') {
        ...
        if (name === 'ISTEXT')
            return typeof v === 'string' && !isErr(v);
        if (name === 'ISBLANK')
            return v === null;
        if (name === 'ISNA')
            return v === E_NA;
    ...
    if (name === 'INDEX') {
        ...
        if (cn === 0)
            cn = 1;
        if (rn < 1 || rn > arr.rows || cn < 1 || cn > arr.cols)
            return E_REF;
    ...
    if (name === 'MATCH') {
        ...
        for (let i = 0; i < arr.vals.length; i++) {
            ...
            if (kind > 0 ? compare('<=', v, tgt) : compare('>=', v, tgt))
                best = i + 1;
        }
    ...
    if (['SUM', 'MAX', 'MIN', 'ABS', 'INT', 'PRODUCT', 'AVERAGE', 'COUNT'].includes(name)) {
        ...
        if (name === 'COUNT')
            return nums.length;
        if (name === 'SUM')
            return nums.length ? nums.reduce((a, b) => a + b, 0) : 0.0;
        if (name === 'PRODUCT')
            return nums.reduce((p, v) => p * v, 1.0);
        if (name === 'AVERAGE')
            return nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : E_DIV;
        if (!nums.length)
            return 0.0;
        if (name === 'MAX')
