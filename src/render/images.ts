...
const PASSTHROUGH: Record<string, LoadedImage['extension']> = {
  'image/png': 'png',
  'image/jpeg': 'jpeg',
  'image/jpg': 'jpeg',
  'image/gif': 'gif',
 ...
 */
export function attachmentIdFromPath(path: string): number | null {
...
function toEmbeddable(mimetype: string, bytes: Buffer): LoadedImage | null {
  const mime = mimetype.toLowerCase();

  const direct = PASSTHROUGH[mime];
  if (direct) return { buffer: bytes, extension: direct };
 ...
 */
export async function loadFigureImages(
  odoo: OdooClient,
  model: RenderModel,
  ...
  for (const sheet of model.sheets) {
    for (const fig of sheet.figures) {
      ...
      if (paths) paths.push(fig.path);
      else pathById.set(id, [fig.path]);
    }
  }
  ...
  try {
    records = await odoo.call<Array<Record<string, unknown>>>('ir.attachment', 'search_read', [
      [['id', 'in', ids]],
      ['id', 'mimetype', 'name'],
    ]);
  } catch (e) {
  ...
  const usable: number[] = [];
  const mimeById = new Map<number, string>();
  ...
  const missing = ids.filter((id) => !records.some((r) => (r.id as number) === id));

  if (usable.length) {
    try {
      const payloads = await odoo.call<Array<Record<string, unknown>>>('ir.attachment', 'read', [
        usable,
        ['id', 'datas'],
      ]);
      ...
      for (const rec of payloads) {
        ...
        if (typeof datas !== 'string' || !datas) continue;
        ...
        for (const path of pathById.get(id) ?? []) out.set(path, image);
      }
    } catch (e) {
  ...
  if (missing.length || wrongType.length || unparseable) {
    logger.warning(
      `Figure images: ${resolved}/${pathById.size} resolved. ` +
