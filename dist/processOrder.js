...
async function createAttachment(odoo, orderId, file) {
    const created = await odoo.call('ir.attachment', 'create', [
        [
            {
                name: file.name,
...
export async function processOrder(orderId, orderName, spreadsheetIdIn) {
    ...
    let spreadsheetId = spreadsheetIdIn;
    // 1. Fetch order state and spreadsheet_id.
    try {
        const orderData = await odoo.call('sale.order', 'read', [[orderId], ['state', 'spreadsheet_id']]);
        ...
        if (state !== 'sale') {
            logger.info(`Skipping order ${orderId} because state is '${state}' (must be confirmed 'sale').`);
            return;
        }
        if (!spreadsheetId) {
...
export function decideWebhook(payload) {
    ...
    return {
        action: 'process',
        orderId,
        orderName: payload.display_name || payload.name || `SO${orderId}`,
